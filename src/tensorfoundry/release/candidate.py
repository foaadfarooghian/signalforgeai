"""Offline-first release-candidate evidence gate for TensorFoundry."""
...
from tensorfoundry.distillation.check import run_distillation_check
...
def _write_mock_sft_run(
    *,
    ...
    (output_dir / "adapter_model.safetensors").write_text(
        "tensorfoundry mock sft adapter\n",
        encoding="utf-8",
    )
    started_at = utc_now()
    payload = build_training_run(
        ...
        config={
            "source": "tensorfoundry-release-candidate-check",
...
def _write_mock_dpo_run(
    *,
    ...
    output_dir.mkdir(parents=True, exist_ok=True)
    (output_dir / "adapter_model.safetensors").write_text(
        "tensorfoundry mock dpo adapter\n",
        encoding="utf-8",
    )
    started_at = utc_now()
    payload = build_training_run(
...
def format_release_candidate_markdown(payload: Mapping[str, Any]) -> str:
    ...
    lines = [
        "# TensorFoundry Release Candidate",
        "",
...
def main(argv: Optional[List[str]] = None) -> int:
    parser = argparse.ArgumentParser(
        description="Run TensorFoundry release-candidate evidence checks."
    )
