...
def validate_openai_pricing(required_models: Optional[list[str]] = None) -> int:
    ...
    print(f"[pricing] ✅ loaded {len(pricing)} model price entries from: "
          f"{os.getenv('TENSORFOUNDRY_OPENAI_PRICING_PATH') or 'package default'}")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Validate TensorFoundry OpenAI pricing config.")
    parser.add_argument(
        "--require",
        action="append",
        default=[],
