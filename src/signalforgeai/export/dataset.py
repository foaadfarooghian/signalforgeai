"""
Dataset exporter for SignalForge AI.
...
from signalforgeai.export.extract import (
    extract_instruction,
    extract_response,
    extract_step_prompt_full, 
    extract_step_text_full
)
from signalforgeai.export._json import dumps_row
from signalforgeai.export.quality import attach_split_meta
# ----------------------------
# Models
# ----------------------------
...
def export_sft(
    *,
    ...
    if not step:
        step = os.getenv("SIGNALFORGEAI_SFT_STEP", "critic_check")
    if min_response_chars is None:
        min_response_chars = int(os.getenv("SIGNALFORGEAI_MIN_RESPONSE_CHARS", "200"))
...
def main(argv: Optional[List[str]] = None) -> int:
    p = argparse.ArgumentParser(description="Export SFT datasets from SignalForge AI logs.")
