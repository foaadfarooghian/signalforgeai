"""OpenTelemetry-compatible JSON export for SignalForge AI traces."""
...
def trace_events_to_otel(events: List[Dict[str, Any]]) -> Dict[str, Any]:
    """Map SignalForge AI trace events to an OTel JSON-shaped span list."""
        ...
        "resourceSpans": [
            {
                "resource": {
                    "attributes": [
                        {
                            "key": "service.name",
                            "value": {"stringValue": "signalforgeai"},
                        }
                    ]
                },
                "scopeSpans": [
                    {
                        "scope": {"name": "signalforgeai.trace_export", "version": "trace.v0"},
                        "spans": spans,
                    }
                ],
            }
...
def main(argv: Optional[list[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Export a SignalForge AI trace as OTel JSON.")
