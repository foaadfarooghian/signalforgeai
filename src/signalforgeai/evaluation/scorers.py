...
def resolve_scorer(name: str) -> ScorerFn:
    ...
    if ":" in name:
        if not _allow_scoring_imports():
            raise ValueError(
                "Import-based scoring is disabled. Set SIGNALFORGEAI_ALLOW_SCORING_IMPORTS=1 to enable."
            )
...
def _allow_scoring_imports() -> bool:
    return os.getenv("SIGNALFORGEAI_ALLOW_SCORING_IMPORTS", "0").lower() in {"1", "true", "yes"}
