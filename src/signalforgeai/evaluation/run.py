...
from signalforgeai.learning.routing_policy import load_policy_if_present
from signalforgeai.learning.bandits import (
    RoutingBanditsV0,
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    parser = argparse.ArgumentParser(description="Run a SignalForge AI evaluation suite.")
    ...
    parser.add_argument("--bandits", type=str, default="learning_ops/bandits/routing_bandits_v0.json", help="Path to bandit state file",)
    parser.add_argument("--no-bandits", action="store_true", help="Disable bandit routing",)
    parser.add_argument("--stats", type=str, default="learning_ops/bandits/routing_stats_v0.json", help="Path to routing stats file (cost/latency EWMA)")
    ...
    chosen_model: str = os.getenv("SIGNALFORGEAI_MODEL_ID") or "unknown"
    bandits = None
    candidates: list[str] = []
    ...
    if policy:
        ...
        os.environ["SIGNALFORGEAI_MODEL_ID"] = chosen_model
        ...
        if not candidates:
            ...
            chosen_model = bandits.choose_model(
                ...
                candidates,
                stats=stats,
                max_cost_usd=float(os.getenv("SIGNALFORGEAI_MAX_COST_USD", "0")) or None,
                max_latency_s=float(os.getenv("SIGNALFORGEAI_MAX_LATENCY_S", "0")) or None,
                utility_lambda_cost=float(os.getenv("SIGNALFORGEAI_UTILITY_LAMBDA_COST", "0.0")),
                utility_mu_latency=float(os.getenv("SIGNALFORGEAI_UTILITY_MU_LATENCY", "0.0")),
            )
            os.environ["SIGNALFORGEAI_MODEL_ID"] = chosen_model
